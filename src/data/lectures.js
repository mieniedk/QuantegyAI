...
const LECTURES = {
  ...
  'c008': {
    ...
    keyIdea: 'Exponential: y = abˣ. Logarithm is the inverse: log_b(y) = x means bˣ = y. Growth when b > 1, decay when 0 < b < 1.',
    steps: [
      { title: 'Exponential models', content: 'Compound interest: A = P(1 + r/n)^(nt). Continuous growth: A = Peʳᵗ.' },
      { title: 'Log properties', content: 'log(ab) = log a + log b. log(a/b) = log a − log b. log(aⁿ) = n·log a.' },
      { title: 'Solving exponential equations', content: 'Isolate the exponential term, take log of both sides, solve for the variable.' },
    ],
    example: { problem: 'Solve 2ˣ = 32.', solution: ['32 = 2⁵.', 'So 2ˣ = 2⁵ → x = 5.'], answer: 'x = 5' },
  ...
  'c010': {
    ...
    keyIdea: 'The derivative measures instantaneous rate of change. The integral accumulates area. The Fundamental Theorem of Calculus links them: ∫ₐᵇ f(x)dx = F(b) − F(a).',
    steps: [
      { title: 'Limits', content: 'lim(x→a) f(x) = L means f(x) approaches L as x approaches a. Factor or rationalize to evaluate 0/0 forms.' },
      { title: 'Derivatives', content: 'Power rule: d/dx[xⁿ] = nxⁿ⁻¹. Product, quotient, and chain rules handle compositions. f\'(x) = 0 at critical points.' },
      { title: 'Integrals', content: 'Antiderivative reverses differentiation. ∫xⁿ dx = xⁿ⁺¹/(n+1) + C. Definite integrals compute net area.' },
    ],
    example: { problem: 'Find d/dx[3x⁴ − 2x + 7].', solution: ['Power rule: 4·3x³ − 2 + 0.', '= 12x³ − 2.'], answer: '12x³ − 2' },
