...
const LECTURES = {
  ...
  'c008': {
    ...
    keyIdea: 'Exponential: f(x) = a·b^x (initial value a, base b). Transforms: y = a·b^(x−h) + k shifts h horizontally and k vertically, scales by |a|; horizontal asymptote y = k. Logarithm is the inverse: log₍b₎(y) = x ⟺ b^x = y (with b > 0, b ≠ 1). Log graphs: y = a·log₍b₎(x−h) + k has vertical asymptote x = h, domain x > h.',
    steps: [
      ...
      { title: 'Log properties', content: 'log(ab) = log a + log b. log(a/b) = log a − log b. log(a^n) = n·log a. Change of base: log₍b₎(x) = (ln x)/(ln b).' },
      { title: 'Solving equations', content: 'Exponential: match bases and set exponents equal, or take ln (or log₍b₎) of both sides. Log equations: rewrite in exponential form b^y = x, then solve; check domain (argument of log must be positive).' },
    ],
